...
export default function Page({ title, displayTitle, description, image, sections }) {
	...
	for (let index = 0; index < sections.length; index++) {
		...
		if (data.style !== "full" && data.image) {
			image = <div className={
				...
				+
				(data.style === "split-left" ? " right-0 bg-top-left" : " left-0 bg-right-top")
			}
		...
		elements.push(<div
			className={"relative lg:min-h-[23vw] py-6 lg:py-12 " + (data.colors === "dark" ? " bg-gray-800 text-white" : " bg-white text-gray-800")}
			key={index}>
			{image}
			<Container className="left relative z-10">
				<div className={
					data.style === "full" ?
						"w-full" : data.style === "split-left" ?
							"lg:w-1/2" : "lg:w-1/2 lg:ml-auto"
				}>
					<GenericContent className={data.colors === "dark" ? "theme-dark" : "theme-light"}>
